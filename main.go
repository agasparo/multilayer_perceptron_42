...
func Train(Network network.Net, Doing graphical.GoTo, final int, TL file.Learn, SaveData network.Save) {
	...
	err := network.Train(x_train, y_train, epochs, learning_rate, Network, &SaveData, Doing.Decrease)
...
func Predict(Network network.Net, TL file.Learn) {
	...
	for i := len(TL.Datas) / 2; i < len(TL.Datas); i++ {

		for e := 0; e < len(TL.Datas[i]); e++ {
			x = append(x, TL.Datas[i][e])
		}
	}
	x_train := mat.NewDense(len(TL.Datas[0]), len(TL.Datas) / 2 + 1, x)
	real_data := TL.Response[len(TL.Response) / 2:len(TL.Response)]
